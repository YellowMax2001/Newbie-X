...
splitted_targets: $(patsubst %.c, %.c.o, $(filter %.c, ${SplittedCompileSrcFiles})) \
 $(patsubst %.cpp, %.cpp.o, $(filter %.cpp, ${SplittedCompileSrcFiles}))
	...
	$(COMPILE_OPTION_PREFIX) $(foreach c_o, \
		$(filter %.c.o, $^), \
		echo "Out: $(notdir $(patsubst %.c.o, %.a, ${c_o}))"; \
		$(Compile_AR) rcs -o $(patsubst %.c.o, %.a, ${c_o}) -shared ${c_o} $(strip ${CompileCCLinkFlags});)
	$(COMPILE_OPTION_PREFIX) $(foreach cpp_o, \
		$(filter %.cpp.o, $^), \
		echo "Out: $(notdir $(patsubst %.cpp.o, %.a, ${cpp_o}))"; \
		$(Compile_AR) rcs -o $(patsubst %.cpp.o, %.a, ${cpp_o}) -shared ${cpp_o} $(strip $(CompileCPPLinkFlags));)
...
ifeq (${TmpVarNormalCppFiles},)
	$(COMPILE_OPTION_PREFIX) $(Compile_AR) rcs -o $@ $^ $(strip $(CompileCCFlags))
else
	$(COMPILE_OPTION_PREFIX) $(Compile_AR) rcs -o $@ $^ $(strip $(CompileCPPFlags))
endif
...
distclean:: clean
	...
	@$(call rm_files, $(patsubst %.cpp, %.a, $(filter %.cpp, ${SplittedCompileSrcFiles})))
	@$(call rm_files, $(patsubst %.c, %.a, $(filter %.c, ${SplittedCompileSrcFiles})))
